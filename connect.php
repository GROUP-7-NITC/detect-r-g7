...
class Database{        //class database contains basic functionalities
	...
	public function authenticate_user($user_name,$pass,$tbl_name)     //function to authenticate user 
	{
      ...
      public function notification($email,$title,$message)    //function to send email to respective user
      {
...
class Comments extends Database{      //class comments
 public function __construct(){
  ...
  public function make_comments($page,$note,$comm,$id){    //function to make comments to user
     $sql="insert into comment(location,notes,comments,tracking_id)values('$page','$note','$comm','$id')";
     $result=mysqli_query($this->link,$sql);
  }
}
